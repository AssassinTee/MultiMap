...
void CTilesetMapper::Proceed(CEditorMap2::CLayer *pLayer, int ConfigID, RECTi Area)
{
	if(ConfigID < 0 || ConfigID >= m_aRuleSets.size())
		return;
			...
			for(int i = 0; i < pConf->m_aRules.size(); ++i)
			{
				...
				for(int j = 0; j < pConf->m_aRules[i].m_aConditions.size() && RespectRules; ++j)
				{
					...
					else
					{
						...
						{
							if(pLayer->m_aTiles[CheckIndex].m_Index > 0 && pCondition->m_Value == CRuleCondition::EMPTY)
								RespectRules = false;

							if(pLayer->m_aTiles[CheckIndex].m_Index == 0 && pCondition->m_Value == CRuleCondition::FULL)
								RespectRules = false;
						}
						else
						{
							if(pLayer->m_aTiles[CheckIndex].m_Index != pCondition->m_Value)
								RespectRules = false;
						}
					}
				}
...
void CDoodadsMapper::AnalyzeGameLayer()
{
	...
	CEditorMap2::CLayer *pLayer = &(m_pEditor->m_aLayers[m_pEditor->m_GameLayerID]);//m_Map.m_pGameLayer;
...
void CDoodadsMapper::PlaceDoodads(CEditorMap2::CLayer *pLayer, CRule *pRule, array<array<int> > *pPositions, int Amount, int LeftWall)
{
			...
			if(pRule->m_HFlip)
			{
				for(int y = 0; y < pRule->m_Size.y; y++)
					for(int x = 0; x < pRule->m_Size.x/2; x++)
					{
						...
						CTile Tmp = pLayer->m_aTiles[Index];
						pLayer->m_aTiles[Index] = pLayer->m_aTiles[CheckIndex];
						pLayer->m_aTiles[CheckIndex] = Tmp;
					}
...
void CDoodadsMapper::Proceed(CEditorMap2::CLayer *pLayer, int ConfigID, int Amount)
{
	if(ConfigID < 0 || ConfigID >= m_aRuleSets.size())
		return;
	...
	{
		pLayer->m_aTiles[i].m_Index = 0;
		pLayer->m_aTiles[i].m_Flags = 0;
	}
